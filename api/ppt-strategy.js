...
async function callClaude(key, systemPrompt, userContent, maxTokens) {
  const body = { model: MODEL, max_tokens: maxTokens, messages: [{ role: 'user', content: userContent }] };
...
export default async function handler(req, res) {
  ...
  const { mode = 'strategy', ebgAnalysis, brand, store, references = [],
          title, prompt, ebgContext, referenceSummary } = req.body || {};
...

{
  ...
  "sections": [
    { "title": "섹션 제목", "points": ["포인트1 (구체적, 1~2줄)", "포인트2", "포인트3"] },
    { "title": "섹션 제목2", "points": ["포인트1", "포인트2"] }
  ],
  "keyMetrics": [{"label": "지표명", "value": "수치 또는 내용"}],
  "conclusion": "결론 또는 액션 아이템 한 줄"
}

sections는 2~3개, points는 2~4개. keyMetrics는 수치 데이터가 있을 때만 포함.
반드시 JSON만 반환하세요. 마크다운 코드블록 없이 순수 JSON으로 응답하세요.`;

      const r = await callClaude(key, systemPrompt,
        `슬라이드 주제/제목: ${title || '커스텀 분석'}\n\n프롬프트/내용:\n${prompt}`, 1500);
    ...
    const stratPrompt = `다음은 ${brand || '브랜드'} 브랜드의 키맨 EBG(External Brand Group) 미팅 분석 결과입니다.
...
반드시 JSON만 반환하세요. 마크다운 코드블록(\`\`\`) 없이 순수 JSON으로만 응답하세요.`;
    ...
    const r = await callClaude(key, null, stratPrompt, 2048);
    if (!r.ok) {
      const err = await r.json().catch(() => ({}));
      return res.status(r.status).json({ error: err.error?.message || `Claude API 오류 (${r.status})` });
    }
